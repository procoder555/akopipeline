...
def get_season(date):
    ...
    # Define the season ranges
    if (month == 3 and day >= 21) or (3 < month < 6) or (month == 6 and day <= 20) or (month == 6 and day >= 21) or (6 < month < 9) or (month == 9 and day <= 20):
        return "2023SS"
    else:
        return "2023FW"
...
class Pipeline:
    ...
    def run(self):
        ...
        ac_df = pd.read_parquet(r'ac.parquet')
        pos_df = pd.read_parquet(r'pos.parquet')
        sku_df = pd.read_parquet(r'sku.parquet')
        trans_df = pd.read_parquet(r'transac.parquet')
        ...
        selected_cols['actual_season'] = selected_cols['date'].apply(get_actual_season)
        selected_cols['season'] = selected_cols['date'].apply(get_season)
        ...
        final_df = sel_multiple.rename(columns={'MondayOfWeek': 'ds', 'qty': 'y', 'item_prediscount_price': 'item_value__w','ac':'unique_id','family_old':'family','family2_old':'family2'})
        grouped_df = final_df.groupby(['ds','unique_id','season','sales_season','actual_season','iscovid','is_pub_holidays','color','is_active','gender','family','family2','article']).agg({
            'y': 'sum',
            'item_value__w': 'mean',
            'nopendays':'mean',
            'discount_perc' : 'mean'
        })
        final = grouped_df.reset_index()
        print(final.columns)
        final.to_parquet("output_data/processed_data.parquet")
